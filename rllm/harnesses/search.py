"""SearchHarness: multi-turn ReAct search + browse loop for data tasks.

A minimal web-research agent matching how frontier models are evaluated on
search-heavy benchmarks like GAIA (cf. HuggingFace Open Deep Research and
Princeton HAL, the two reference GAIA scaffolds): a web **search** + page
**browse** loop using rLLM's Tavily tools — a single ``TAVILY_API_KEY``
covers both.

Registered as the built-in ``"search"`` agent (``registry/agents.json``) and
the ``default_agent`` for the search-heavy catalog datasets (``gaia``,
``browsecomp``, ``hle_search``, ``seal0``, ``widesearch``), so
``rllm eval gaia`` resolves an agent out of the box.

Code-execution / file-parsing / multimodal tools are intentionally out of
scope: GAIA's transform (``rllm.data.transforms:gaia_transform``) targets the
text-only subset (file-attachment tasks are skipped). Those tools are the
natural follow-up for the harder GAIA levels.
...
# The agent prompt MUST tell the model it has tools and to use them — search
# benchmarks need fresh facts, and most models won't call tools unless
...
def run_tool_loop(client, model: str, tools: list, question: str, *, system_prompt: str = AGENT_SYSTEM_PROMPT, max_turns: int = MAX_TURNS) -> tuple[list[Step], str]:
    ...
    Factored out from `SearchHarness.run` so the control flow is unit-testable
    with a fake client + fake tools (no network / no API keys).
    """
...
class SearchHarness:
    ...
    No sandbox required. Pure ``AgentFlow`` — produces an Episode whose
    ``artifacts["answer"]`` is the final answer (extracted by reward_fns
    like ``gaia_reward_fn``).
    """
